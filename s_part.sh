...
pacman -Rns xdg-utser-dirs --noconfirm &> /dev/null
...
dd if=/dev/zero of=/swap/swapfile bs=1M count=$swsize status=progress &> /dev/null
chmod 600 /swap/swapfile
mkswap /swap/swapfile &> /dev/null
swapon /swap/swapfile &> /dev/null
echo "/swap/swapfile          none            swap            defaults        0 0" >> /etc/fstab
gcc -O2 /btrfs_map_physical.c -o /tmp/btrfs_map_physical &> /dev/null
